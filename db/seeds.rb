...
Blog.create!(
  ...
  content: %q(
    ...
    <p>정말 그런 수준의 대규모 <strong>역사 왜곡</strong>이 가능했다면, 그건 오히려 우리 민족만 특별히 당한 것이 아니라 전 세계 역사상 유례없는 일이다. 그리고 그런 조작에 수백 년, 수천 년 동안 아무도 제대로 대응하지 못했다는 식의 논리는 결국 우리 스스로를 지나치게 무능하고 멍청한 존재로 만드는 셈이다.</p>
